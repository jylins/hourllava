...
from transformers.trainer import is_accelerate_available, has_length, is_datasets_available, is_sagemaker_mp_enabled, get_parameter_names, ALL_LAYERNORM_LAYERS, logger
from transformers.trainer_utils import seed_worker

from hourllava.util.misc import rank0_print
from hourllava.dataset.sampler import LengthGroupedSampler
from hourllava.util import get_mm_adapter_state_maybe_zero_3
...
class LLaVATrainer(Trainer):
    ...
    def __init__(self, log_dir, streaming, *args, **kwargs):
        super(LLaVATrainer, self).__init__(*args, **kwargs)

        self.log_file = os.path.join(log_dir, f'training_log_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.txt')
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.streaming and isinstance(self.train_dataset, StreamingDataset) and self.state.is_world_process_zero:
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        super(LLaVATrainer, self)._load_optimizer_and_scheduler(checkpoint)

        # Resume dataloader
        if checkpoint is not None and self.streaming: # and self.args.streaming_dataset:
            try:
