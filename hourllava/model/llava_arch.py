...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_max_len(self, max_len):
        max_len = torch.tensor(max_len, device=self.device)
        torch.distributed.all_reduce(max_len, op=torch.distributed.ReduceOp.MAX)
        max_len = max_len.item()
        
        if max_len <= 8192:
            max_len = 8192
        elif max_len > 8192 and max_len <= 16384:
            max_len = 16384
        elif max_len > 16384 and max_len <= 20480:
            max_len = 20480
        elif max_len > 20480 and max_len <= 24576:
            max_len = 24576
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            if isinstance(images, list) and not isinstance(images[0], dict):
            ...
            image_features = []
            if images[0].shape[-1] == 1152:
                for idx, image_feat in enumerate(images):
                    tmp_feat = self.get_model().mm_projector(image_feat).flatten(1,2)
                    if tmp_feat.shape[0] < 64:
                        sampled_frames = np.linspace(0, tmp_feat.shape[0] - 1, 64, dtype=int)
                        tmp_feat = tmp_feat[sampled_frames]
                    image_features.append(tmp_feat)
                del images
                torch.cuda.empty_cache()
            else:
                # Standardize images to have 4 dimensions (B, C, H, W)
        ...
        # get the max_len over different nodes
        max_len = max([x.shape[0] for x in new_input_embeds])
        # max_len = self.get_max_len(max([x.shape[0] for x in new_input_embeds]))
