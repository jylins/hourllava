...
def get_datum(item):
    if item.get('video', None):
        video_path = os.path.join(ARGS.root_path, item['data_source'], item['video']).replace('.mp4', '.pt').replace('VideoMarathon/videos', 'VideoMarathon/features')
        if not os.path.exists(video_path):
            print(f'Video not found: {video_path}')
            return None
        sample = {"video": video_path, "text": item}
    else:
...
def main():
    ...
    print(f'Number of videos: {len(video_set)}')
    print(f'Number of samples: {len(dataset)}')
