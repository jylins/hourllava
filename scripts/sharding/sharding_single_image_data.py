...
def get_datum(item):
    """Build one sample."""
    if item.get('image'):
        img_path = os.path.join(ARGS.root_path, ARGS.image_folder, item['image'])
        img = Image.open(img_path).convert('RGB')
        return {"image": img, "text": item}
    return {"image": Image.fromarray(np.random.randint(0, 256, (1, 1, 3), np.uint8)), "text": item}
...
def main():
    ...
    orininal_dataset_len = len(dataset)
        ...
        if item.get('image', None) is not None:
            for conv in item['conversations']:
                if '<image>' in conv['value']:
                    ...
                    break
            if flag:
                if os.path.exists(os.path.join(args.root_path, args.image_folder, item['image'])):
            ...
            if args.exclude_text == 0:
                new_dataset.append(item)
            
    dataset = new_dataset

    print(f'Number of samples with images: {len(dataset)} / {orininal_dataset_len}')
