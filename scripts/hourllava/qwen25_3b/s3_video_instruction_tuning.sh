...
DATA_PATH="./experiments/streaming/hourllava_s3.yaml"
...
S1_RUN_NAME="hourllava-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-s1"
echo "S1_RUN_NAME: ${S1_RUN_NAME}"

########### Stage 2: Video-Language Adaptation ############
S2_RUN_NAME="hourllava-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-s2"

########### Stage 3: Video Instruction Tuning #############
RUN_NAME="hourllava-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-s3"
PREV_STAGE_CHECKPOINT="./work_dirs/hourllava/${S2_RUN_NAME}"
...
HSA_FORCE_FINE_GRAIN_PCIE=1 OMP_NUM_THREADS=16 NCCL_DEBUG=INFO torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --master_addr="${ADDR}" --node_rank="${RANK}" --master_port="${PORT}" \
    ...
    --mm_tunable_parts="mm_mlp_adapter,mm_vision_resampler,mm_language_model" \
